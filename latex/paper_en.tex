...
Figure~\ref{fig:robot} shows the evolution of automation capital intensity (ICT GFCF per employee) across ABM sectors in the OECD panel.
BPO/SSC exhibits the highest and most volatile ICT capital intensity, consistent with its role as the primary digital automation vector.
Manufacturing, Retail/Services, and Public show lower but growing intensity, while Healthcare and Agriculture remain near zero.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_01_robot_density_trends.png}
\caption{Automation capital intensity (ICT GFCF per employee) by ABM sector (OECD mean, 1970--2023)}
\label{fig:robot}
\end{figure}
...
\subsection{Calibrated vs Empirical: All Methods}

Figure~\ref{fig:oecdpl} compares calibrated $\sigma$ (Papers~01--02) with GMM and Bayesian estimates for all six sectors, with 95\% confidence/credible intervals.
The Bayesian estimates are systematically lower than GMM for market sectors, reflecting the shrinkage toward meta-analytic priors.
Manufacturing shows the largest discrepancy between methods ($\hat{\sigma}_{\text{GMM}}=4.88$ vs $\hat{\sigma}_{\text{Bayes}}=2.03$), driven by the informative prior ($\mu=1.5$).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_06_oecd_vs_poland.png}
\caption{Calibrated vs empirical $\sigma$: all methods with 95\% CI/HDI. Non-market sectors marked ``prior''.}
\label{fig:oecdpl}
\end{figure}
